...
from mathematical_functions import rand_arg_min, rand_arg_max, compute_vote_vectors
...
class PWC(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, n_classes, metric='rbf', n_neighbors=None, probabilistic=False,
        ...
        if self.metric not in KERNEL_PARAMS.keys() and self.metric != PWC.PRECOMPUTED:
        ...
        self.combine_labels = combine_labels
        if not isinstance(self.combine_labels, bool):
            raise TypeError("combine_labels must be boolean")
    ...
    def fit(self, X, y, c=None):
        ...
        if np.size(X) > 0:
            ...
            self.V_ = compute_vote_vectors(y=y, c=c, n_unique_votes=self.n_classes,
            ...
            if self.combine_labels:
                ...
                temp[np.arange(len(self.V_)), rand_arg_max(self.V_, random_state=self.random_state, axis=1)] = 1
    ...
    def predict_proba(self, X, normalize=True):
        ...
        if self.metric == PWC.PRECOMPUTED:
            K = X
        else:
            K = pairwise_kernels(X, self.X_, metric=self.metric, **self.kwargs_)

        if self.n_neighbors is None:
            # calculating labeling frequency estimates
            P = K @ self.V_
        else:
            if np.size(self.X_, 0) < self.n_neighbors:
    ...
    def predict(self, X, C=None):
        ...
        return rand_arg_min(arr=np.dot(P, C), axis=1, random_state=self.random_state)